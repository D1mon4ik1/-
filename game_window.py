...
frame_per_second = 32
tower_image = 'images/other/tower.png'
background_image = 'images/other/bg-game.png'
floor_image = 'images/other/down.png'

framepersecond_clock = pygame.time.Clock()
window = pygame.display.set_mode((window_width, window_height))

game_images = {'score_images': [pygame.image.load('images/number/{}.png'.format(i)).convert_alpha() for i in range(10)]}
dragon_images = [pygame.image.load('images/dragon_green/small/dragon{}.png'.format(i)).convert_alpha() for i in range(1, 9)]
...
dragon_skins = {
    'skin1': [pygame.image.load('images/dragon_green/small/dragon{}.png'.format(i)).convert_alpha() for i in range(1, 9)],
    'skin2': [pygame.image.load('images/dragon_blue/small/dragon{}.png'.format(i)).convert_alpha() for i in range(1, 9)],
    'skin3': [pygame.image.load('images/dragon_red/small/dragon{}.png'.format(i)).convert_alpha() for i in range(1, 9)],
}
...
def draw_pause_button(window):
    ...
    window.blit(pause_button_image, (window_width - pause_button_image.get_width() - 10, 10))
    return pygame.Rect(window_width - pause_button_image.get_width() - 10, 10, pause_button_image.get_width(), pause_button_image.get_height())
...
def draw_pause_menu(window):
    ...
    menu_rect = pygame.Rect((window_width - 300) // 2, (window_height - 300) // 2, 300, 300)
    ...
    text_rect = pause_text.get_rect(center=(window_width // 2, 150))
    ...
    continue_text_rect = continue_text_render.get_rect(center=(window_width // 2, (window_height - 300) // 2 + 150))
    ...
    main_menu_text_rect = main_menu_text_render.get_rect(center=(window_width // 2, (window_height - 300) // 2 + 250))
...
def draw_game_over_menu(window, score):
    ...
    menu_rect = pygame.Rect((window_width - 300) // 2, (window_height - 350) // 2, 300, 350)
    window.blit(transparent_background, (0, 0))
    pygame.draw.rect(window, (90, 90, 90), menu_rect, border_radius=20)
    ...
    text_rect = game_over_text.get_rect(center=(window_width // 2, (window_height - 350) // 2 + 50))
    window.blit(game_over_text, text_rect)

    font_score = pygame.font.Font(font_path, 35)
    score_text = font_score.render(translate_text("Рахунок", translations) + ": " + str(score), True, (255, 255, 255))
    score_rect = score_text.get_rect(center=(window_width // 2, (window_height - 350) // 2 + 120))
    ...
    restart_text_rect = restart_text_render.get_rect(center=(window_width // 2, (window_height - 350) // 2 + 210))
    ...
    main_menu_text_render = button_font.render(translate_text("Головне меню", translations), True, button_text_color)
    main_menu_text_rect = main_menu_text_render.get_rect(center=(window_width // 2, (window_height - 350) // 2 + 300))
    main_menu_button_rect = main_menu_text_rect.inflate(20, 10)
    pygame.draw.rect(window, button_color, main_menu_button_rect, border_radius=8)
    window.blit(main_menu_text_render, main_menu_text_rect)
...
def dragongame1(dragon_skin=None):
    ...
    while True:
        ...
        if not paused:
            if pygame.time.get_ticks() - last_update_time >= 80:
                last_update_time = pygame.time.get_ticks()
            ...
            if game_over:
                ...
                if result == "restart":
                    return dragongame1(dragon_skin)
                elif result == "menu":
                    return
